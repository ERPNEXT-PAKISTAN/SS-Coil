# Server Script: Supplier Aging API
...
supplier_group_filter = args.get("supplier_group")
...
supplier_names = list(set([entry.get("party") for entry in entries if entry.get("party")]))
supplier_map = {}
if supplier_names:
    ...
    for supplier_doc in suppliers:
        supplier_map[supplier_doc.get("name")] = supplier_doc

supplier_rows = {}
for entry in entries:
    supplier = entry.get("party")
    if not supplier:
        continue

    supplier_doc = supplier_map.get(supplier) or {}
    supplier_group = supplier_doc.get("supplier_group") or "Ungrouped"
    if supplier_group_filter and supplier_group != supplier_group_filter:
        continue

    if supplier not in supplier_rows:
        supplier_rows[supplier] = {
            "supplier": supplier,
            "supplier_name": supplier_doc.get("supplier_name") or supplier,
    ...
    amount = (entry.get("credit") or 0) - (entry.get("debit") or 0)
    if not amount:
        continue

    row = supplier_rows[supplier]
...
rows.sort(key=lambda row: ((row.get("supplier_group") or ""), -(row.get("balance") or 0), row.get("supplier") or ""))
companies = frappe.get_all("Company", pluck="name", order_by="name asc", limit_page_length=10000)
all_supplier_groups = frappe.get_all("Supplier Group", pluck="name", order_by="name asc", limit_page_length=10000)
frappe.response["message"] = {"rows": rows, "companies": companies, "supplier_groups": all_supplier_groups, "as_on_date": str(as_on_date)}
