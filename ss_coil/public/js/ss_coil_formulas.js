...
ss_coil.formulas.formula_block = function ({ seq, title, syntax, calc_html, result_text, note, accent }) {
	const bar = accent || "#2563eb";
	const seqBadge =
		seq != null
			? `<div style="min-width:32px;height:32px;border-radius:10px;background:${bar};color:#fff;font-weight:800;font-size:14px;display:flex;align-items:center;justify-content:center;flex-shrink:0;box-shadow:0 1px 3px rgba(15,23,42,.12);">${ss_coil.formulas.esc(
					String(seq),
				)}</div>`
			: "";
	return `
...
ss_coil.formulas.build_html = function (frm) {
	...
	const addBlock = (opts) => {
		formulaSeq += 1;
		blocks.push(ss_coil.formulas.formula_block({ ...opts, seq: formulaSeq }));
	};
	...
	addBlock({
		...
		calc_html: output_rows.length
			? output_rows
					.map((row, idx) => {
						const plus =
							idx > 0 ? `<span style="color:#92400e;font-weight:800;margin:0 4px;">+</span>` : "";
	...
	const totalFormulas = formulaSeq;

	return `
		<div class="ss-coil-formulas" style="max-width:960px;padding:4px 0 24px;">
			<div style="display:flex;align-items:baseline;flex-wrap:wrap;gap:10px;margin-bottom:4px;">
				<div style="font-size:18px;font-weight:800;color:#102a43;">${__("SS Coil formulas")}</div>
				<div style="font-size:12px;font-weight:700;color:#1e40af;background:#dbeafe;border:1px solid #93c5fd;border-radius:8px;padding:4px 10px;">
					${__("{0} formulas", [totalFormulas])}
				</div>
			</div>
